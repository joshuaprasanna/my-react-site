...
function ModalPopups() {
  ...
  const handleShow = () => dispatch({ type: "SHOWPOPUP" });

  function setValue(event) {
    // alert(event.target.getAttribute('data-value'));
    modalState.formik.formik.setFieldValue(
      modalState.modalData.fieldValue,
  ...
  return (
    <>
      <Modal show={modalState.showModal} onHide={handleClose}>
        ...
        <Modal.Body>
          <ul class="list-group">
            {modalState.modalData.body.map((obj) => {
              return (
                <li
                  class="list-group-item"
                  data-value={obj.displayText}
                  onClick={setValue}
                >
                  {obj.displayText}
                </li>
